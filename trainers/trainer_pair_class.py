...
class Trainer:
    ...
    def forward_pass(self, enc_premise, enc_hypothesis):
    ...
    def train(self,epochs):
        ...
        for epoch in range(epochs):
            ...
            for enc_premise, enc_hypothesis, targets in self.train_loader:
                logits = self.forward_pass(enc_premise, enc_hypothesis)
    ...
    def validate(self):
        ...
        total_val_loss = 0
        with torch.no_grad():
            for enc_premise, enc_hypothesis, targets in self.val_loader:
                targets = targets.to(self.device)
                logits = self.forward_pass(enc_premise, enc_hypothesis)
                loss = self.criterion(logits, targets)
